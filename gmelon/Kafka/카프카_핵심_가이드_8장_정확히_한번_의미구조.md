...
- 트랜잭션 API 를 사용하는 코드 흐름에 따른 알고리즘 동작 확인
    1. 프로듀서의 `initTransaction()` 호출
        1. 트랜잭션 프로듀서의 트랜잭선 코디네이터 역할을 맡을 브로커로 보내짐
            1. 브로커는 전체 프로듀서의 트랜잭션 코디네이터 역할을 나눠 맡음
        2. 코디네이터에 새 트랜잭션 ID 를 등록하거나, 기존 ID 의 에포크를 증가시킴
            1. (좀비가 된 기존 프로듀서를 펜싱하기 위해)
    2. beginTransaction() 호출
        1. 프로듀서에 현재 진행 중인 트랜잭션이 있음을 알려줌
            1. 브로커는 아직 트랜잭션 시작 여부를 모름
        2. 프로듀서가 레코드 전송을 시작하면, 파티션에 레코드 전송 시마다 `AddPartitionsToTxn` 요청을 보냄
            1. 브로커는 이 프로듀서에 진행 중인 트랜잭션이 있으며 레코드가 추가되는 파티션들이 트랜잭션의 일부임을 알게 됨
            2. 이 정보는 트랜잭션 로그에도 기록됨
        3. sendOffsetsToTrasaction() 호출 시, 트랜잭션 코디네이터로 오프셋 & 컨슈머그룹ID 가 포함된 요청 전송
            1. 코디네이터는 컨슈머 그룹 코디네이터를 찾은 뒤, 컨슈머 그룹이 보통 하는 것처럼 오프셋 커밋
        4. commit/abortTransaction() 호출 시, 코디네이터에 `EndTxn` 요청이 전송
            1. 코디네이터는 트랜잭션 로그에 커밋 & 중단 시도를 기록
            2. 만약 로그에는 써졌지만 실제로 트랜잭션 커밋 & 중단이 실패하면, 새로운 트랜잭션 코디네이터가 선출되어 이어서 트랜잭션을 완료 처리하게 됨
            3. [`trasaction.timeout.ms`](http://trasaction.timeout.ms) 내에 트랜잭션이 커밋 & 중단 되지 않으면 트랜잭션 코디네이터가 자동으로 트랜잭션을 중단시킴
...
CREATE INDEX idx_inbox_processed_at ON message_inbox(processed_at);
*/
...
// 4. Kafka Consumer with Inbox Pattern
...
public class OrderEventConsumer {
    ...
    public OrderEventConsumer(MessageInboxRepository inboxRepository, 
    ...
    public void processOrderEvent(ConsumerRecord<String, String> record) {
        try {
            ...
            String messageId = event.getEventId(); // 또는 record.key() 사용
            ...
            inbox.setOffset(record.offset());
            inbox.setProcessedAt(LocalDateTime.now());
            inbox.setPayload(record.value());
    ...
    private void processOrder(OrderEvent event) {
        ...
        switch (event.getEventType()) {
            case "ORDER_CREATED":
                ...
                break;
            case "ORDER_UPDATED":
                orderService.updateOrder(event.getOrder());
                break;
...
public class OrderEvent {
    private String eventId;  // 고유 ID (UUID)
...
public class OrderEventProducer {
    ...
    public void publishOrderEvent(Order order, String eventType) {
        try {
            ...
            event.setEventId(UUID.randomUUID().toString()); // 고유 ID 생성
            ...
            kafkaTemplate.send("order-events", event.getEventId(), payload)
                .addCallback(
...
public class InboxCleanupJob {
    ...
    public void cleanupOldMessages() {
        ...
        int deleted = inboxRepository.deleteByProcessedAtBefore(cutoffDate);
        log.info("Deleted {} old inbox messages", deleted);
    }
}
```