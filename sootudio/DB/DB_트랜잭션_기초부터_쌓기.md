...
> Lock이란, 동시의 여러 트랜잭션이 같은 데이터를 건드릴 때 데이터가 꼬이지 않게 하는 일종의 안전벨트라고 생각하면 됩니다.

Lock의 종류에는 간단하게 아래와 같은 2 가지가 있습니다.
- S(Shared, 읽기 잠금): 여러 트랜잭션이 동시에 읽기는 함께 가능. (쓰기와는 충돌)
- X(Exclusive, 쓰기 잠금): 쓰기 전용. 잡는 동안 다른 읽기/쓰기와 충돌.
