...
여담으로, JOIN 방법을 사용하려 임시 테이블을 만들어서 작업하려고 했었는데, 회사 내부 정책 때문에 테이블 생성을 못 해서 결국 `WHERE IN` 방식을 사용했습니다...
...
여기서 나오는 `BEGIN` 이라는, 새로운 키워드가 나옵니다.
처음에는 생소한 키워드라서 헷갈렸는데, 이 키워드가 바로 트랜잭션을 시작하는 키워드입니다. 
트랜잭션에 대해서는 새로운 문서를 만들어서 작성하겠습니다.
